...
export async function validateBuild(
  ...
  try {
    ...
    const isDefaultPage =
      html.includes("OpenSandbox Ready") ||
...
export function extractMissingPackages(error: any): string[] {
  ...
  const importMatches = message.matchAll(
  ...
  const moduleMatches = message.matchAll(
  ...
  const packageMatches = message.matchAll(
    /Package ["']([^"']+)["'] not found/g,
  );
...
export function classifyError(error: any): ErrorType {
  ...
  if (message.includes("vite") || message.includes("compilation")) {
    return "vite-error";
  }
