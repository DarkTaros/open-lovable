...
const SESSION_STORAGE_PREFIX = "Novable:generation-session:";
...
const getSessionStorageKey = (sandboxId: string) =>
  `${SESSION_STORAGE_PREFIX}${sandboxId}`;

const reviveChatMessages = (
...
function AISandboxPage() {
  ...
  const restoreSavedSession = (sandboxId: string) => {
    try {
      ...
      setConversationContext({
        ...
        scrapedWebsites: (
          savedSession.conversationContext.scrapedWebsites || []
        ).map((site) => ({
          ...site,
          timestamp: new Date(site.timestamp),
        })),
        appliedCode: (savedSession.conversationContext.appliedCode || []).map(
          (entry) => ({
            ...entry,
  ...
  const persistCurrentSession = (activeSandboxId: string) => {
    try {
      ...
      localStorage.setItem(
        getSessionStorageKey(activeSandboxId),
  ...
  const restoreSandbox = async (sandboxId: string) => {
    if (sandboxCreationRef.current) {
      console.log(
        "[restoreSandbox] Sandbox operation already in progress, skipping...",
      );
      return null;
    }
...
Tip: I automatically detect and install npm packages from your code imports (like react-router-dom, axios, etc.)`,
  ...
  const fetchSandboxFiles = async (
    overrideSandboxData?: SandboxData | null,
  ) => {
                              ...
                              if (node.file) {
                                ...
                                return (
                                  <div
                                    ...
                                    onClick={() =>
                                      handleFileClick(node.file!.path)
                                    }
                                  >
        ...
        while (true) {
          ...
          for (const line of lines) {
            if (line.startsWith("data: ")) {
              ...
              if (data.type === "error") {
                streamErrorMessage =
                  data.error || data.message || "Failed to generate recreation";
                break;
              }
      ...
      <div className="font-sans bg-background text-foreground h-screen flex flex-col">
        <div className="bg-white py-[15px] py-[8px] border-b border-border-faint flex items-center justify-between shadow-sm">
          ...
          <div className="flex items-center gap-2">
            ...
            appConfig.ai.availableModels.length > 1 ? (
              <select
                value={aiModel}
                onChange={(e) => {
